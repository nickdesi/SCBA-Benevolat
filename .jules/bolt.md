...
## 2026-08-01 - Avoid synchronous blocking operations inside React .map() rendering loops
**Learning:** Calling synchronous blocking methods like `localStorage.getItem` and `JSON.parse` inside a `.map()` loop during the render phase of a frequently re-rendered component (like `VolunteerSlot`) causes a severe N+1 performance bottleneck. It stalls the main thread and introduces scroll jank. However, replacing this with a `useState` lazy initializer to read once per component mount creates a functional bug: if the parent component re-uses the component instance with different props, the state does not automatically re-initialize, leading to stale data being displayed.
**Action:** Always hoist expensive synchronous operations out of `.map()` iteration loops. To properly handle derived state and maintain reactivity when props change without causing a stale state bug, calculate the value *once per render* in the main body of the component, or use a `useMemo` hook with the correct dependency array instead of `useState`.

## 2026-08-01 - Avoid synchronous blocking operations inside React .map() rendering loops (Addendum)
**Learning:** When hoisting synchronous `localStorage` reads (like `getMyRegistrations()` or `getStoredName()`) out of the rendering loop to prevent input lag, using `useState` lazy initializers (`useState(() => getStoredName())`) or `useMemo` with an empty dependency array (`[]`) can introduce a stale state bug. If the underlying local storage data changes (e.g., after the user signs up), the component will not re-evaluate the memoized value, causing UI mismatches (e.g., failing to recognize the user's registration).
**Action:** When wrapping synchronous storage reads in `useMemo`, strictly pass the appropriate dynamic dependencies that naturally correlate with the data change. For example, use `currentCount` (derived from the optimistic roster length) in `VolunteerSlot` or `[entries]` (the Firebase carpool entries list) in `CarpoolingSection`. This ensures the hook re-reads `localStorage` precisely when an update has occurred, preventing both N+1 rendering bottlenecks and stale state bugs.
...
## 2026-11-20 - Avoid redundant array traversals and Date instantiations in JSX loops when child component already memoizes it
**Learning:** Checking a condition with an inline `array.some(...)` wrapping a child component (like `<MatchTicker />`) inside the parent `App.tsx` render loop creates redundant O(N) array traversals, `new Date()` object allocations, and GC pressure *on every single render pass* of the application. The child component already correctly uses a memoized O(N) early-exit loop (`useMemo`) to find the next item and returns `null` if the condition isn't met.
**Action:** Remove the inline `.some()` guard from the JSX and let the child component's internal memoized logic naturally return `null`. This moves the check from a heavy O(N) per-render operation to an efficient O(N) evaluation that only recalculates when dependencies change.
...
## 2026-11-20 - Avoid complex string reverse parsing and Date allocations inside controlled form renders
**Learning:** In controlled components like `GameForm.tsx` where state updates trigger a re-render on every keystroke, keeping inline calculations like `min={new Date().toISOString().split('T')[0]}` or reverse string parsers (like `getISODate(formattedString)`) inside the JSX causes severe input lag. It results in $O(N)$ redundant string manipulations and Date object allocations precisely when user interaction is fastest.
**Action:** Instead of parsing a formatted display string back into ISO format on every render cycle, always maintain and directly bind the primitive ISO state (`formData.dateISO`) to the input value. Additionally, use `useMemo(() => getTodayISO(), [])` to cache invariant date boundaries (like `min` values) on component mount to completely eliminate date allocations during typing. Hoist any `Intl.DateTimeFormat` objects triggered by input changes to module scope to avoid re-instantiation.
