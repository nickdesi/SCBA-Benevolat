...
## 2025-10-23 - Use string slicing instead of Date parsing for iteration cache keys
**Learning:** In components that group items by month/year (like `groupGamesByMonth` in `GameList`), allocating `new Date(game.dateISO)` for every game inside the mapping loop just to derive the month/year cache key causes O(N) object allocations and creates GC pressure, especially for large datasets.
**Action:** When a date string is already in a strictly formatted format like ISO (e.g., "YYYY-MM-DD"), use fast string slicing (e.g., `game.dateISO.substring(0, 7)`) to construct the cache key. Only parse the date into a Date object on cache miss when formatting is actually required. This reduces Date object allocations from O(N) to O(K) where K is the number of unique groups.
