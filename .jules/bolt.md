...
## 2026-05-13 - Refactoring in AdminStats
**Learning:** The `AdminStats` component contained an invalid TypeScript syntax issue where `missingRoles` and `isComplete` were declared twice, including once as a trailing property declaration without a comma separating it from the property before. Also, it recalculated `isGameFullyStaffed` and `getMissingRoles` while those values were already available in `roleStats` through `getGameRoleStats`, causing an O(N) array iteration per game unnecessarilly.
**Action:** Refactored the property map generation and removed the `getMissingRoles` and `isGameFullyStaffed` re-calculations by re-using the derived stats in `roleStats` instead. Ensured type checks pass.
