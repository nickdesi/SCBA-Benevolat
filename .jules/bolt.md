...
## 2026-10-25 - Avoid Date creation for ISO strings inside rendering loops
**Learning:** `new Date(game.dateISO)` creates a local time date object at midnight UTC. This forces an offset bug for timezones west of UTC, causing the date to jump backwards to the previous day (e.g. `2025-11-15` becomes `Nov 14` at 19:00 EST). Additionally, `Date` instantiation inside `useMemo` loops like `games.forEach` or `[].map` inside React component pipelines creates redundant object allocation and garbage collection.
**Action:** When extracting components of an ISO string formatted as "YYYY-MM-DD", avoid `new Date(dateISO)`. Instead, simply extract the year and month by splitting the string (`const [year, month] = dateISO.split("-")`), and reconstruct the Date by passing year/month explicitly to `new Date(year, monthIndex, 1)` or just manually map to string lookup tables if possible.
