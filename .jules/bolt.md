...
## 2026-11-20 - Avoid O(N) Date object instantiation in array grouping logic by using string slicing
**Learning:** Instantiating `new Date(game.dateISO)` inside the `groupGamesByMonth` loop in `GameList.tsx` for every single game just to extract the year and month causes redundant O(N) object allocations and garbage collection pressure during React renders.
**Action:** When a date string is strictly formatted (like 'YYYY-MM-DD' for ISO dates), use fast string slicing (e.g. `dateISO.substring(0, 7)`) to generate cache keys directly. Only fall back to `new Date()` instantiations when there's a cache miss and formatting is actually required.

## 2026-11-20 - Avoid O(N) Date object instantiation in array grouping logic by using string slicing
**Learning:** Instantiating `new Date(game.dateISO)` inside the `groupGamesByMonth` loop in `GameList.tsx` for every single game just to extract the year and month causes redundant O(N) object allocations and garbage collection pressure during React renders. Furthermore, because JavaScript parses `'YYYY-MM-DD'` strings as UTC midnight, extracting local components (`getFullYear()`, `getMonth()`) from this object creates an off-by-one day bug for any user in a timezone behind UTC (e.g., UTC-5).
**Action:** When a date string is strictly formatted (like 'YYYY-MM-DD' for ISO dates), use fast string slicing (e.g. `dateISO.substring(0, 7)`) to safely construct cache keys. Only fall back to `new Date()` instantiations when there's a cache miss and formatting is actually required. When falling back to `new Date`, use string splitting `dateISO.split('-')` and manually construct the Date with `new Date(year, monthIndex, 15)` to avoid timezone boundary drift completely.
