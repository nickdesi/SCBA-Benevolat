...
## 2026-11-20 - Pre-compute string lowercasing before React list rendering
**Learning:** Re-evaluating `storedName.toLowerCase()` inside a React render `.map()` loop (e.g. for checking if a listed driver or passenger is the current user in `CarpoolingSection.tsx`) forces redundant O(N) string allocations during every render pass. This strains the garbage collector and adds unnecessary CPU overhead, especially on memory-constrained devices.
**Action:** Use a `useMemo` hook at the component level to pre-compute the lowercased string once per render cycle (e.g., `const normalizedStoredName = useMemo(() => (storedName || '').toLowerCase(), [storedName]);`), and reference this static value inside the `.map()` loops.
