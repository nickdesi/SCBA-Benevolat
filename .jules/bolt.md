...
## 2026-08-01 - Avoid synchronous I/O and JSON parsing in render loops of controlled components
**Learning:** Calling synchronous storage methods like `localStorage.getItem` combined with `JSON.parse` inside a component's render function is a severe bottleneck, especially when that component has controlled inputs (like a text field) that trigger re-renders on every keystroke. This causes massive input lag on lower-end devices. Furthermore, using expensive array operations like `.some` combined with inline `.toUpperCase()` on every keystroke aggravates the problem.
**Action:** Always hoist expensive array definitions, string normalization, and synchronous I/O out of the render loop. Use `useMemo` to evaluate these operations exactly once when their dependencies change, shielding the UI from redundant O(N) operations and blocking I/O during rapid re-renders like typing.
