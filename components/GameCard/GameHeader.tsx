...
import { Home, Plane, Car, Flame, AlertTriangle } from 'lucide-react';
...
const GameHeader: React.FC<GameHeaderProps> = ({
    ...
    return (
        <div className={`relative p-4 overflow-hidden ${isHomeGame
            ...
            {/* Watermark Icon */}
            <div className={`absolute -right-4 -top-4 opacity-10 dark:opacity-[0.12] dark:text-slate-400 select-none pointer-events-none transition-opacity`}>
                {isHomeGame ? <Home className="w-24 h-24" /> : <Plane className="w-24 h-24" />}
            </div>
            ...
            <div className="relative flex items-start justify-between gap-2 mb-1 z-10">
                ...
                <div className="flex flex-wrap gap-1.5 items-center flex-shrink-0">
                    <span className={`
                        ...
                        `}>
                        {isHomeGame ? <><Home className="w-3 h-3 inline mr-1" />Domicile</> : <><Car className="w-3 h-3 inline mr-1" />Extérieur</>}
                    </span>
                            ...
                            if (remainingSeats < 0) {
                                ...
                                return (
                                    <span className="px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide rounded-full bg-amber-100 text-amber-700 dark:bg-amber-900/60 dark:text-amber-300 border border-amber-200 dark:border-amber-700 animate-pulse">
                                        <AlertTriangle className="w-3 h-3 inline mr-0.5" /> {deficit} cherch. place
                                    </span>
                                );
                            }
                            ...
                            if (remainingSeats > 0) {
                                return (
                                    <span className="px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide rounded-full bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800">
                                        <Car className="w-3 h-3 inline mr-0.5" /> {remainingSeats} place{remainingSeats > 1 ? 's' : ''} dispo
                                    </span>
                                );
                            }
                    ...
                    {isUrgent && !isFullyStaffed && (
                        ...
                        >
                            <Flame className="w-3 h-3 inline mr-0.5" /> Urgence
                        </motion.span>
                    )}
                </div>
            </div>
            ...
            <div className="text-sm text-slate-500 dark:text-slate-400 space-y-3">
                ...
                <div className="pt-1">
                    <a
                        ...
                        className="group flex items-center justify-between sm:justify-start gap-3 w-full sm:w-auto px-4 py-2.5 min-h-[44px] bg-slate-50 hover:bg-slate-100 dark:bg-slate-800/50 dark:hover:bg-slate-800 border-l-4 border-blue-500 hover:border-blue-600 rounded-r-xl transition-all shadow-sm hover:shadow"
                    >
