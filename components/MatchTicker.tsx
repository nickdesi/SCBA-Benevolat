        ...
        <div className="bg-slate-900 border-b border-slate-800 text-white overflow-hidden py-2 relative z-30">
            <div className="flex items-center animate-marquee whitespace-nowrap hover:pause">
                <div className="flex gap-8 items-center px-4">
                    <span className="font-bold text-amber-400 uppercase tracking-widest text-xs">⚡ À VENIR :</span>
                    {upcomingGames.map((game, i) => (
                        <div key={i} className="flex items-center gap-2 text-sm font-medium">
                            ...
                            <span className="text-white font-bold">{game.team}</span>
                            <span className="text-slate-500">vs</span>
                            <span className="text-slate-200">{game.opponent}</span>
                            <span className="text-slate-600 mx-2">|</span>
                        </div>
                    ))}
                    {/* Duplicate specifically to insure smooth loop is usually done via CSS or duplicating content enough times. 
                        For simplicity here, we just display the list. 
                        Ideally for true infinite scroll we need duplicate content.
                    */}
                    {upcomingGames.map((game, i) => (
