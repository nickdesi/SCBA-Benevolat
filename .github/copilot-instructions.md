...
# 🏀 Source Unique de Vérité & Adresses FFBB (Voir AGENTS.md)

1. **API First & Zéro Inférence Client** : Ne jamais chercher à "deviner" ou inférer des villes/adresses depuis les noms d'équipes côté client. Corriger toujours à la source dans `ffbb-data-client`.
2. **Résolution Salles FFBB** : Dans `ffbb-data-client`, croiser obligatoirement la salle Directus, la cartographie Meilisearch (`cartographie_id = S-{id}`) et l'organisme hôte pour garantir le format `Nom, Rue, CP VILLE`.
3. **Action Directe & Zéro Bouclage** : Diagnostiquer, corriger immédiatement de manière ciblée, sans suites de tests globales bloquantes.
