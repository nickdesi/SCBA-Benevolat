...
export const StatCard: React.FC<StatCardProps> = ({ label, value, icon, gradient, colSpan }) => {
  return (
    ...
    >
      ...
      <div
        className={`absolute -top-12 -right-12 w-32 h-32 bg-gradient-to-br ${gradient} rounded-full opacity-5 blur-3xl group-hover:opacity-20 transition-opacity duration-500`}
      />

      {/* Content */}
      <div className="relative z-10 flex items-center gap-4">
        <div
          className={`w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-gradient-to-br ${gradient} flex items-center justify-center shadow-lg shadow-indigo-500/20 transform -rotate-3 group-hover:rotate-0 transition-transform duration-500`}
        >
      ...
      {/* Bottom Accent Line */}
      <div
        className={`absolute bottom-0 left-0 h-[2px] w-0 group-hover:w-full bg-gradient-to-r ${gradient} transition-all duration-500 opacity-50`}
      />
    </motion.div>
  );
};
