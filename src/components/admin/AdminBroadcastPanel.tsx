...
export const AdminBroadcastPanel: React.FC<AdminBroadcastPanelProps> = ({ onToast }) => {
  ...
  const getTypeStyles = (t: string) => {
    switch (t) {
      case 'urgent':
        return 'bg-red-600 border-red-500 shadow-red-500/30';
      case 'warning':
        return 'bg-orange-500 border-orange-400 shadow-orange-500/30';
      default:
        return 'bg-blue-600 border-blue-500 shadow-blue-500/30';
    }
  };
            ...
            {message && (
              ...
              >
                <div className="p-4 bg-slate-50 dark:bg-slate-900/30 rounded-xl border border-slate-200 dark:border-slate-700 border-dashed">
                  ...
                  <div
                    className={`p-4 rounded-xl flex items-center gap-3 text-white shadow-lg ${getTypeStyles(type)}`}
                  >
                    {getTypeIcon(type)}
