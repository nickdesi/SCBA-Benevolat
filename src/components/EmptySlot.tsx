...
const EmptySlot: React.FC<EmptySlotProps> = ({ onClick, label = 'Rejoindre', isUrgent }) => {
  return (
    <motion.button
      ...
      aria-label={`${label} ce poste bénévole`}
    >
      ...
      <div
        className={`
          ...
          ${
            isUrgent
              ? 'border-red-500/80 bg-red-500/10 text-red-600 dark:border-red-400 dark:bg-red-950/40 dark:text-red-300 shadow-[0_0_14px_rgba(239,68,68,0.25)]'
      ...
      <span
        className={`
          text-xs sm:text-sm tracking-tight px-2.5 py-0.5 rounded-full transition-all duration-200
          ${
            isUrgent
              ? 'bg-red-100 text-red-800 dark:bg-red-950/70 dark:text-red-200 border border-red-200/80 dark:border-red-800/80 shadow-xs font-black'
              : 'text-slate-700 dark:text-slate-200 group-hover:text-[#3629e1] dark:group-hover:text-indigo-300 font-bold'
          }
        `}
      >
        {label}
