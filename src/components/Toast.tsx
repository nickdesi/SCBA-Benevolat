...
const ToastItem: React.FC<ToastItemProps> = memo(({ toast, onClose }) => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`flex-shrink-0 p-1.5 rounded-full bg-white/20 ${currentStyle.icon}`}
        aria-hidden="true"
      >
        {icons[toast.type]}
      </div>
      ...
      {/* Message */}
      <span className={`font-semibold text-sm flex-1 ${currentStyle.text}`}>{toast.message}</span>
