  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-start gap-3 mb-3">
        ...
        {/* Percentage Badge */}
        <span
          className={`px-3 py-1.5 text-sm font-black rounded-xl ${getBadgeStyles(game.percent)} shrink-0 transition-transform duration-200 hover:scale-105`}
        >
          {game.percent}%
        </span>
      </div>
      ...
      {game.missingRoles.length > 0 && (
        <div className="mb-3">
          <p className="text-xs">
            ...
            <span className="text-red-500 dark:text-red-300">{game.missingRoles.join(', ')}</span>
          </p>
        </div>
      )}
      ...
      <div className="relative">
        <div className="w-full h-2.5 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden">
          <div
            ...
            className={`h-full bg-gradient-to-r ${getProgressColor(game.percent)} rounded-full transition-all duration-500 ease-out`}
          />
        </div>
...
const AdminStats: React.FC<AdminStatsProps> = ({ games, onClose, onToast }) => {
  ...
  const stats = useMemo(() => {
    ...
    const getComingWeekend = () => {
      ...
      const nextSaturday = new Date(d.setDate(diff + 5));
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 bg-slate-900/95 z-[100] flex items-center justify-center p-3 md:p-6"
        onClick={onClose}
      >
        <motion.div
        ...
        >
          ...
          <div className="relative bg-gradient-to-br from-slate-800 via-slate-900 to-slate-950 p-5 md:p-6 text-white shrink-0 overflow-hidden">
            ...
            <motion.div
              ...
              className="relative z-10 flex gap-2 mt-5 bg-slate-800 p-1.5 rounded-2xl border border-slate-700/50"
            >
              <button
                ...
                >
                  ...
                  >
                    ...
                    <KPICard
                      icon={Calendar}
                      label="CE WEEK-END"
                      value={`${stats.weekendStats.percent}%`}
                      subValue={`${stats.weekendStats.filled}/${stats.weekendStats.total} postes`}
                      color="indigo"
                    />
                    <KPICard
                      icon={Users}
                  ...
                  >
                    <button
                      ...
                      className={`shrink-0 px-4 py-2 text-xs font-bold rounded-full transition-all flex items-center gap-1.5 ${
                        filter === 'all'
