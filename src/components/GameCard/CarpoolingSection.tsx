...
const CarpoolingSection: React.FC<CarpoolingSectionProps> = memo(
  ({
    ...
    // ⚡ Bolt Optimization: Pre-compute lowercase name once per render cycle
    // to prevent redundant O(N) string reallocations in inner rendering loops (.map).
    const normalizedStoredName = useMemo(() => (storedName || '').toLowerCase(), [storedName]);
    ...
    const handleRemove = useCallback(
      (entryId: string, entryName: string) => {
        if (entryName.toLowerCase() === normalizedStoredName) {
    ...
    const canRequestSeat =
      currentUserEntry?.type === 'passenger' &&
        ...
        <div className="flex items-center justify-between">
          ...
          {entries.length > 0 && (
            <span className="text-xs text-slate-400 dark:text-slate-500">
              {drivers.length} conducteur{drivers.length > 1 ? 's' : ''} • {passengers.length}{' '}
            ...
            >
              ...
              <div className="space-y-2">
                {drivers.map((driver) => {
                  ...
                  const isCurrentUser = driver.name.toLowerCase() === normalizedStoredName;
                      ...
                      <div className="p-3">
                        ...
                        <div className="flex items-center justify-between mb-2">
                          ...
                          >
                            ...
                            <span>
                              {remainingSeats} place{remainingSeats > 1 ? 's' : ''}
                            </span>
                          </div>
                        </div>
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            {(isCurrentUser || isAdmin) && (
                              <button
                                ...
                                className="w-11 h-11 flex items-center justify-center text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-colors cursor-pointer active:bg-red-100"
                        ...
                        {isCurrentUser && pendingRequests.length > 0 && (
                          ...
                          >
                            <p className="text-[11px] font-semibold text-amber-600 dark:text-amber-400 uppercase mb-2 flex items-center gap-1">
                              ...
                              {pendingRequests.map((request) => (
                                ...
                                >
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[11px] text-amber-600 dark:text-amber-400 bg-amber-100 dark:bg-amber-800/40 px-1.5 py-0.5 rounded">
                                      {request.seats || 1} pl.
                                    </span>
                                  </div>
                                  <div className="flex items-center gap-1">
                                    <motion.button
                                      ...
                                      className="w-11 h-11 flex items-center justify-center bg-emerald-500 text-white rounded-xl hover:bg-emerald-600 transition-colors cursor-pointer active:bg-emerald-700"
                                      aria-label="Accepter"
                                    >
                                    ...
                                    <motion.button
                                      ...
                                      className="w-11 h-11 flex items-center justify-center bg-red-500 text-white rounded-xl hover:bg-red-600 transition-colors cursor-pointer active:bg-red-700"
                                      aria-label="Refuser"
                                    >
                                      <X className="w-4 h-4" />
                                    </motion.button>
                        ...
                        {driver.matchedWith && driver.matchedWith.length > 0 && (
                          <motion.div
                            ...
                            className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-700"
                          >
                            <p className="text-[11px] font-semibold text-emerald-600 dark:text-emerald-400 uppercase mb-2 flex items-center gap-1">
            ...
            >
              ...
              <div className="space-y-2">
                {passengers.map((passenger) => {
                  const isCurrentUser = passenger.name.toLowerCase() === normalizedStoredName;
                      ...
                      <div className="p-3">
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2 min-w-0 flex-1">
                            ...
                            <span className="px-1.5 py-0.5 bg-amber-100 dark:bg-amber-800/40 text-amber-700 dark:text-amber-300 text-[11px] font-semibold rounded-md">
                          ...
                          {/* Status Badge */}
                          {passenger.status === 'matched' && (
                            <span className="flex items-center gap-1 px-2 py-1 bg-emerald-500 text-white text-[11px] font-bold rounded-full">
                              <Check className="w-3 h-3" />
                              Confirmé
                            </span>
                          )}
                          {passenger.status === 'pending' && (
                            <span className="flex items-center gap-1 px-2 py-1 bg-amber-500 text-white text-[11px] font-bold rounded-full animate-pulse">
                              <Clock className="w-3 h-3" />
                              En attente
                            </span>
                          )}
                        ...
                        {matchedDriver && (
                          <div className="mt-2 flex items-center gap-1.5 text-xs text-emerald-600 dark:text-emerald-400">
                            ...
                            <span>
                              Avec <strong>{matchedDriver.name}</strong>
                            </span>
                            {matchedDriver.phone && (
                              <span className="ml-2">
                        ...
                        {passenger.status === 'pending' && requestedDriver && (
                          <div className="mt-2 flex items-center gap-1.5 text-xs text-amber-600 dark:text-amber-400">
                            <Clock className="w-3 h-3" />
                            <span>
                              Demande envoyée à <strong>{requestedDriver.name}</strong>
                            </span>
                          </div>
                        )}
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            {isCurrentUser && passenger.status === 'pending' && onCancelRequest && (
                              <motion.button
                                ...
                                className="flex items-center gap-1 px-3 py-2 min-h-[44px] text-amber-600 dark:text-amber-400 bg-amber-100 dark:bg-amber-900/30
                                                                text-xs font-medium rounded-xl hover:bg-amber-200 dark:hover:bg-amber-800/40 transition-colors cursor-pointer active:bg-amber-200"
                              >
                                <X className="w-3 h-3" />
                                Annuler
                              </motion.button>
                        ...
                        {isCurrentUser &&
                          ...
                          availableDrivers.length > 0 && (
                            ...
                            >
                              <p className="text-[11px] font-semibold text-indigo-600 dark:text-indigo-400 uppercase mb-2 flex items-center gap-1">
                                <Sparkles className="w-3 h-3" />
                                ...
                                {availableDrivers.map((driver) => (
                                  ...
                                  >
                                    ...
                                    <motion.button
                                      ...
                                      className="flex items-center gap-1 px-4 py-2 min-h-[44px] bg-gradient-to-r from-indigo-500 to-purple-600
                                                                        text-white text-xs font-semibold rounded-xl shadow-sm
                                                                        hover:from-indigo-600 hover:to-purple-700 transition-all cursor-pointer active:bg-indigo-700"
                                    >
                                      Demander
                                      <ChevronRight className="w-3 h-3" />
                                    </motion.button>
