...
export const useGames = (options: UseGamesOptions): UseGamesReturn => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      matchesQuery,
      (snapshot) => {
        const matchesData: Game[] = snapshot.docs.map(
          (docSnap) =>
            ({
              id: docSnap.id,
              ...docSnap.data(),
  ...
  const { teams, allTeams, uniqueLocations, uniqueOpponents, filteredGames } = useGameFilters({
    games: sortedGames,
    selectedTeam,
    currentView,
    favoriteTeams,
  ...
  const buildDefaultRoles = (team: string) => {
    ...
    const applicableRoles = DEFAULT_ROLES.filter((role) => !(role.name === 'Goûter' && isSenior));
