...
export const useAppActions = (props: UseAppActionsProps) => {
  ...
  const addGameWithToast = useCallback(
  ...
  const updateGameWithToast = useCallback(
    async (game: Game) => {
      try {
  ...
  const deleteGameWithToast = useCallback(
    async (id: string) => {
      try {
        await deleteGame(id);
        return true;
  ...
  const volunteerWithToast = useCallback(
    async (gameId: string, roleId: string, name: string | string[]) => {
      if (!isAuthenticated) {
  ...
  const updateVolunteerWithToast = useCallback(
    async (gameId: string, roleId: string, old: string, next: string) => {
      try {
  ...
  const addCarpoolWithToast = useCallback(
    async (gameId: string, entry: Omit<CarpoolEntry, 'id'>) => {
      if (!isAuthenticated) {
  ...
  return {
    ...
    requestSeatWithToast: async (gId: string, pId: string, dId: string) => {
      try {
        await handleRequestSeat(gId, pId, dId);
        addToast('✨ Demande envoyée !', 'success');
      } catch {
        addToast('Erreur demande', 'error');
      }
    },
    ...
    rejectPassengerWithToast: async (gId: string, dId: string, pId: string) => {
      try {
        await handleRejectPassenger(gId, dId, pId);
        addToast('Demande refusée', 'info');
      } catch {
        addToast('Erreur refus', 'error');
      }
    },
    cancelRequestWithToast: async (gId: string, pId: string) => {
      try {
        await handleCancelRequest(gId, pId);
